...
from .neural_networks import darknet
...
class ServerXavier:
    ...
    def __bbox_cord(self):

        box = BoundingBox(res[0], res[1], res[2], res[3])
        # box = BoundingBox(random.random(), random.random(), random.random(), random.random())
    ...
    def __handle_client(self, conn):
        ...
        while True:
            ...
            elif data == 'get_bbox_cord':
                conn.send(self.__bbox_cord())
            else:
                break
